  ...
  methods: {
    ...
    changeSortType(type) {
      if (["new", "price"].indexOf(type) !== -1) {
        ...
        this.nowSelectedKeyword = null; // 關鍵字過濾RESET
        this.filterType.keyword = null; // 關鍵字過濾RESET
        this.filterType.brand = null; // 品牌過濾RESET
        this.brandList = this.brandList.map((v) =>
          Object.assign(v, { isChose: false })
        );
        this.$emit("setOnlyBW", true, sortRule);
      } else {
        // AI+宇匯混合
        this.sortType.priceSmallToBig = null;
        this.sortType.sortType = type;
        this.$emit("setOnlyBW", false, { sorting: "RELEVANT" });
      }
    },
    //最新品排序
    sortByNew(fd) {
    ...
    fiterByKeyword(fd) {
      return fd.filter((v) => {
        return v.kids
          ? v.kids.some((k) => k.kcontent === this.filterType.keyword)
          : new RegExp(this.filterType.keyword, "ig").test(v.name);
      });
    },
    //品牌過濾
    fiterByBrand(fd) {
    ...
    composeBrandList() {
      ...
      keywordArr.forEach((v) => {
        if (
          !finalKeywordArr.includes(v.kcontent) &&
          keywordScore[v.kcontent] >= 3
        ) {
  ...
  watch: {
    filterType: {
      handler: function (type) {
        ...
        const { brand, keyword } = this.filterType;
        const isFiltering = brand !== null || keyword !== null;

        // 如果使用過濾品牌或關鍵字 則排序只能回復AI資料方式排列
        if (isFiltering) {
          this.sortType.priceSmallToBig = null;
          this.sortType.sortType = "default";
        }
        this.$emit("changeFilterData", fd, isFiltering, this.sortType);
      },
      deep: true,
    },
  },
